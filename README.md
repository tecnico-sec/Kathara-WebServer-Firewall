...
Nota: ao tentar aceder a `exam_answers.py`, obterá um erro por não estar ligado ao serviço da base de dados. Este erro será corrigido no exercício 2.
...
Agora que temos a nossa aplicação *web* de perguntas de exames a correr no servidor corretamente, vamos observar o comportamento do ponto de vista de um utilizador. 
Recorde a topologia de rede que utilizou no exercício 2 e vamos observar o comportamento a partir do pc1.

1. No pc1, verifique que consegue aceder aos exames passados e não aos futuros, tal como observava no servidor:
```bash
curl `http://<ip do webserver>/cgi-bin/public/past_exam_answers.py`
curl `http://<ip do webserver>/cgi-bin/notpublic/future_exam_answers.py`
```
